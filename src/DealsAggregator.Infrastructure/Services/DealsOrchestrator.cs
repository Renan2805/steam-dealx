...
internal sealed class DealsOrchestrator(IGgDealsClient ggDeals, IItadClient itad) : IDealsOrchestrator
{
        ...
        int steamSubId, string region = "br", CancellationToken ct = default)
    {
        ...
        // Fase 1 — paralela: gg.deals sub prices + lookup UUID via sub/{id}
        var ggDealsTask = ggDeals.GetSubPricesAsync([steamSubId], region, ct);
        var uuidTask    = itad.LookupBySteamSubIdAsync(steamSubId, ct);

        await Task.WhenAll(ggDealsTask, uuidTask);

        var ggDealsPrices = (await ggDealsTask).GetValueOrDefault(steamSubId);
        var uuid          = await uuidTask;
